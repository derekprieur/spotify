...
const upsetProductRecord = async (product: Stripe.Product) => {
...
const createOrRetrieveCustomer = async ({email, uuid}: {email: string, uuid: string}) => {
    const {data, error} = await supabaseAdmin.from('customers').select('stripe_customer_id').eq('uuid', uuid).single()
    ...
    if (error || !data?.stripe_customer_id) {
        ...
        console.log(`Customer inserted: ${uuid}`)
...
const copyBillingDetailsToCustomer = async (uuid: string, payment_method: Stripe.PaymentMethod) => {
    ...
    const {error} = await supabaseAdmin.from('users').update({billing_address: {...address}, payment_method: {...payment_method[payment_method.type]}}).eq('id', uuid)
